...
describe('company data boundary', () => {
  it('activates the company from the authenticated database user, not request headers', () => {
    const companyContext = new RequestCompanyContextService();
    ...
    companyContext.run(() => {
      ...
      expect(companyContext.requireCompanyId()).toBe(COMPANY_A);
    });
  });

  it('does not assign a company to an anonymous request', () => {
    const companyContext = new RequestCompanyContextService();

    companyContext.run(() => {
      expect(companyContext.getCompanyId()).toBeNull();
      expect(() => companyContext.requireCompanyId()).toThrow(UnauthorizedException);
    });
  });
