...
import {
  COMPANY_CONTEXT,
  CompanyContextPort,
} from '@/app/interfaces/company-context.interface';
...
export class HtmlCacheInterceptor implements NestInterceptor {
  // This interceptor only emits HTTP caching headers. It never stores HTML in Redis.
  constructor(
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    const cacheOwner = companyId ? `company:${companyId}` : 'public';
    const cacheKey = `${moduleName}:${cacheOwner}:${route}`;
