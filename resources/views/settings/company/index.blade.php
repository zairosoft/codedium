...
@section('content')
    ...
    <div x-data="sorting">
        <div class="mb-5 flex flex-wrap items-center justify-between gap-4">
            <div class="text-lg font-semibold ltr:sm:text-left rtl:sm:text-right dark:text-white-light">{{ __('companies.company_lists') }}</div>
            <div class="flex flex-wrap items-center justify-between gap-4">
                <ul class="flex text-gray-500 dark:text-white-dark">
                    ...
                    <li class="before:content-['/'] before:px-1.5"><a href="javascript:;" class="text-black dark:text-white-light hover:text-black/70 dark:hover:text-white-light/70">{{ __('settings.companies') }}</a></li>
                </ul>
            </div>
        </div>
        ...
        <div class="fixed inset-0 z-[999] hidden overflow-y-auto bg-[black]/60 px-4" :class="isDeleteModal && '!block'">
            <div class="flex min-h-screen items-center justify-center">
                <div x-show="isDeleteModal" x-transition x-transition.duration.300 @click.outside="isDeleteModal = false"
                    ...
                    <div class="p-5 text-center">
                        ...
                        <div class="mx-auto mt-5 sm:w-3/4">คุณต้องลบรายการนี้ใช่ไหม ?</div>
            ...
            Alpine.data('sorting', () => ({
                ...
                items: [
                    @foreach ($companies as $key => $company)
                        {
                            no: {{ $key + 1 }},
                            dd: '<div class="flex items-center gap-2"> <img src="{{ asset($company->img === null ? 'assets/images/image.png' : 'assets/images/companies/' . $company->img) }}" class="w-9 h-9 max-w-none" alt="user-profile" /></div>',
                            name: '{{ $company->name }}',
                            email: '{{ $company->email }}',
                            tel: '{{ $company->tel }}',
                    ...
                    this.datatable = new simpleDatatables.DataTable('#myTable', {
                        data: {
                            headings: [
                                ...
                                "{{ __('companies.email') }}",
                                "{{ __('companies.phone') }}",
                                "{{ __('companies.tax_id_number') }}",
                                "{{ __('others.view') }}@can('setting update') / {{ __('others.edit') }}@endcan @can('setting delete') / {{ __('others.delete') }}@endcan"
                            ],
                            data: this.dataArr,
                        },
                        perPage: 20,
                ...
                searchItems() {
                    return this.items.filter((d) => (d.name && d.name.toLowerCase().includes(this
                        .searchText)));
                },
                deleteConfirm(item) {
                    setTimeout(() => {
                ...
                deleteItem() {
                    ...
                    fetch("{{ route('setting.company.delete') }}", {
                        method: "DELETE",
                        headers: {
                        ...
                        this.showMessage('ลบสำเร็จ', response['message']);
                    });
                    if (this.itemID) {
                        if (this.itemID != 1) {
