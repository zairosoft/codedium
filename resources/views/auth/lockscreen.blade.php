...
@section('title', __('auth.lock_screen'))
@section('content')
<div class="space-y-5 dark:text-white" action="" method="post">        
    <div class="mx-auto w-full max-w-[440px]">
        <div class="mb-10 flex items-center">
            <div class="flex h-16 w-16 items-end justify-center overflow-hidden rounded-full bg-[#00AB55] ltr:mr-4 rtl:ml-4">
                <img src="{{ asset(auth()->user()->img === null ? "assets/images/user.png" : "assets/images/users/".auth()->user()->img) }}" class="w-full object-cover" alt="images" />
            </div>
            <div class="flex-1">
                <h4 class="text-2xl dark:text-white">{{ ucwords(auth()->user()->name) }}</h4>
                <p class="text-white-dark">{{  __('auth.enter_to_unlock') }}</p>
            </div>
        </div>
        <form class="space-y-5" action="{{ route('lockscreen.authenticate') }}" method="post">
            @csrf
            <div>
