...
<head>
    ...
    <title>{{  __('errors.title.404') }} | {{ config('app.name') }}</title>
    ...
    <div class="main-container min-h-screen text-black dark:text-white-dark">
        <div class="relative flex min-h-screen items-center justify-center overflow-hidden">
            <div class="px-6 py-16 text-center font-semibold before:container before:absolute before:left-1/2 before:aspect-square before:-translate-x-1/2 before:rounded-full before:bg-[linear-gradient(180deg,#4361EE_0%,rgba(67,97,238,0)_50.73%)] before:opacity-10 md:py-20">
                <div class="relative">
                    <img :src="$store.app.theme === 'dark' || $store.app.isDarkMode ? '{{ asset("assets/images/error/dark-404.webp") }}' : '{{ asset("assets/images/error/light-404.webp") }}'" alt="404" class="mx-auto -mt-10 w-full max-w-xs object-cover md:-mt-14 md:max-w-xl" />
                    <p class="mt-5 text-base dark:text-white">{{  __('errors.message.404') }}</p>
