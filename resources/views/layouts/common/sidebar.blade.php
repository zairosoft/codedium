        ...
        <div class="bg-white dark:bg-[#0e1726] h-full">
            <div class="flex justify-between items-center px-4 py-3">
                <a href="{{ url('/') }}" class="main-logo flex items-center shrink-0">
                    <img class="w-8 ml-[5px] flex-none" src="{{ asset('assets/images') }}/{{ config('settings.logo') }}"
                        alt="image" />
                    <span
                        class="text-2xl ltr:ml-1.5 rtl:mr-1.5  font-semibold  align-middle lg:inline dark:text-white-light">{{ config('app.name') }}</span>
                </a>
                <a href="javascript:;"
            ...
            <ul class="perfect-scrollbar relative font-semibold h-[calc(100vh-120px)] overflow-y-auto overflow-x-hidden p-4 py-0"
                ...
                @can('user view')
                    <h2
                        ...
                        <span>{{ __('users.users') }}@can('setting view'){{ __('users.and_settings') }}@endcan</span>
                    </h2>
                @endcan
                @can('user view')
                    <li class="menu nav-item">
                        <button type="button" class="nav-link group" :class="{ 'active': activeDropdown === 'users' }"
                            ...
                            <div class="flex items-center">
                                ...
                                <span
                                    class="ltr:pl-3 rtl:pr-3 text-black dark:text-[#506690] dark:group-hover:text-white-dark">{{ __('users.users') }}</span>
                            </div>
                        ...
                        <ul x-cloak x-show="activeDropdown === 'users'" x-collapse class="sub-menu text-gray-500">
                            @can('role view')
                                <li>
                                    <a href="{{ url('roles') }}">{{ __('users.roles') }}</a>
                                </li>
                            @endcan
                            @can('permission view')
                                <li>
                                    <a href="{{ url('permissions') }}">{{ __('users.permissions') }}</a>
                                </li>
                            @endcan
                            <li>
                                <a href="{{ url('users') }}">{{ __('users.users') }}</a>
                            </li>
                        </ul>
                    </li>
                @endcan
                    ...
                    <li class="menu nav-item">
                        <button type="button" class="nav-link group" :class="{ 'active': activeDropdown === 'settings' }"
                            ...
                            <div class="flex items-center">
                                ...
                                <span
                                    class="ltr:pl-3 rtl:pr-3 text-black dark:text-[#506690] dark:group-hover:text-white-dark">{{ __('settings.settings') }}</span>
                            </div>
                        ...
                        <ul x-cloak x-show="activeDropdown === 'settings'" x-collapse class="sub-menu text-gray-500">
                            <li>
                                <a href="{{ route('setting.general') }}">{{ __('settings.general') }}</a>
                            </li>
                            <li>
                                <a href="{{ route('setting.company') }}">{{ __('settings.companies') }}</a>
                            </li>
                            <li>
                ...
                @if(Route::has('support'))
                    <li class="nav-item">
                        <a href="{{ route('support') }}" class="group cursor-pointer">
                            <div class="flex items-center">
                                <svg class="shrink-0 group-hover:!text-primary" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" xmlns="http://www.w3.org/2000/svg" style="width: 26px; height: 26px;">
                                    <path opacity="0.5"
                                ...
                                <span
                                    class="ltr:pl-3 rtl:pr-3 text-black dark:text-[#506690] dark:group-hover:text-white-dark">{{ __('auth.support') }}</span>
                            </div>
                        </a>
                    </li>
                @endif
                <li class="nav-item">
                    <a class="group cursor-pointer active">
                        <div class="flex items-center w-full">
                            <img class="rounded-full object-cover saturate-50 group-hover:saturate-100"
                                src="{{ asset(auth()->user()->img === null ? 'assets/images/user.png' : 'assets/images/users/' . auth()->user()->img) }}"
                                onclick="window.location = '{{ route('profile') }}'" style="width: 26px; height: 26px;"
                                alt="image">
                            <div class="ltr:pl-3 rtl:pr-3 leading text-black dark:text-[#506690] dark:group-hover:text-white-dark w-9/12"
                                onclick="window.location = '{{ route('profile') }}'">{{ ucwords(auth()->user()->name) }}
                                <div class="text-xs text-default-600  truncate">
                                    {{ auth()->user()->roles->pluck('name')[0] }}
                                </div>
                            </div>
                            <button type="button"
                                class="bg-white-dark/20 dark:bg-dark/40 h-7 w-8 text-default-500 inline-flex rounded items-center justify-center"
                                onclick="window.location = '{{ route('logout') }}';">
                                <svg class="w-4.5 h-4.5 rotate-90 shrink-0"
                                    style="--tw-text-opacity: 1; color: rgb(231 81 90 / var(--tw-text-opacity));"
