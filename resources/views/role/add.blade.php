...
@section('title', __('users.add_role'))
@section('style')
<style>
    .is-invalid {
        --tw-text-opacity: 1;
...
@section('content')
    <div>
        <div class="mb-5 flex flex-wrap items-center justify-between gap-4">
            <div class="text-lg font-semibold ltr:sm:text-left rtl:sm:text-right dark:text-white-light">{{ __('users.add_role') }}</div>
            <div class="flex flex-wrap items-center justify-between gap-4">
                <ul class="flex text-gray-500 dark:text-white-dark">
                    ...
                    <li class="before:content-['/'] before:px-1.5"><a href="javascript:;" class="text-black dark:text-white-light hover:text-black/70 dark:hover:text-white-light/70">{{ __('users.add_role') }}</a></li>
                </ul>
            </div>
        </div>
        <div class="panel">
            <div class="mb-5">
                <form action="{{ url('roles') }}" method="POST">
                    ...
                    <div class="@error('name')has-error @enderror">
                        ...
                        <input id="fullName" type="text" name="name" value="{{ old('name') }}" placeholder="{{ __('users.enter_role') }}" class="form-input" />
                        @if ($errors->has('name'))
                    ...
                    <div class="table-responsive mt-6">
                        <table>
                            <thead>
                                <tr>
                                    <th><input type="checkbox" id="checkboxAll" class="form-checkbox" /></th>
                                    <th>{{ __('users.menu') }}</th>
                                        ...
                                        <td>
                                            ...
                                            <div class="@error('permission')has-error @enderror">
                                                <label style="width: 30%">
                                                    <input name="permission[]" class="form-checkbox {{ $permission->name }}" id="checkboxItem" type="checkbox" value="{{ $permission->id }}">{{ $permission->name }}
                                                </label>
                                            </div>
                                            @empty
