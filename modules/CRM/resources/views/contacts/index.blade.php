    ...
    <div class="flex gap-5 relative xl:h-[calc(100vh_-_150px)] h-full sm:min-h-0" :class="{'min-h-[999px]' : isShowSidebar}">
        ...
        <div class="panel flex-1 overflow-auto">
            ...
            <div class="crm-table pt-14 relative">
                <table id="contactsTable" class="whitespace-nowrap">
                </table>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener("alpine:init", () => {
        Alpine.data("contactsList", () => ({
            ...
            async loadContacts() {
                try {
                    const response = await fetch('{{ route("crm.api.contacts") }}');
            ...
            initializeTable() {
                this.datatable = new simpleDatatables.DataTable('#contactsTable', {
                    data: {
                        headings: [
                            'ID',
                            "Contact",
                    ...
                    columns: [
                        {
                            ...
                            render: function(data, cell, row) {
                                return `<div class="flex items-center gap-2">
                                    ...
                                    <div>
                                        <a href="/crm/contacts/${row.cells[0].data}" class="font-semibold text-primary hover:underline">${data}</a>
                        ...
                        },
                        {
                            ...
                            render: function(data, cell, row) {
                                return `<ul class="flex gap-2">
                                    <li>
                                        <a href="/crm/contacts/${row.cells[0].data}" class="hover:text-info" title="View">
                                    ...
                                    <li>
                                        <a href="/crm/contacts/${row.cells[0].data}/edit" class="hover:text-primary" title="Edit">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
                                    ...
                                    <li>
                                        <a href="javascript:;" class="hover:text-danger" title="Delete" @click="deleteContact(${row.cells[0].data})">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
            ...
            setTableData() {
                ...
                for (let i = 0; i < this.contacts.length; i++) {
                    dataArr.push([
                        ...
                        this.contacts[i].lead_score,
                        this.contacts[i].last_contact,
                        this.contacts[i].assigned_to,
                        ''
                    ]);
                }
