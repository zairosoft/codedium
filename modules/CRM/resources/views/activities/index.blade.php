...
<div x-data="activitiesList">    
    <div class="flex gap-5 relative xl:h-[calc(100vh_-_150px)] h-full sm:min-h-0" :class="{'min-h-[999px]' : isShowSidebar}">
        ...
        <div class="perfect-scrollbar panel p-4 flex-none overflow-x-hidden max-w-xs w-full absolute xl:relative z-10 space-y-4 h-full hidden xl:block" :class="isShowSidebar && '!block'">
            ...
            <div>
                <h6 class="text-base font-bold pb-5">Activity Type</h6>
                <ul class="space-y-1.5">
                    <li>
                        <a href="javascript:" class="inline-flex items-center gap-1">
                            <input type="checkbox" id="task" class="form-checkbox" />
                            <label for="task" class="mb-0 cursor-pointer">Task</label>
                        </a>
                    </li>
                    <li>
                        <a href="javascript:" class="inline-flex items-center gap-1">
                            <input type="checkbox" id="call" class="form-checkbox" />
                            <label for="call" class="mb-0 cursor-pointer">Call</label>
                        </a>
                    </li>
                    <li>
                        <a href="javascript:" class="inline-flex items-center gap-1">
                            <input type="checkbox" id="meeting" class="form-checkbox" />
                            <label for="meeting" class="mb-0 cursor-pointer">Meeting</label>
                        </a>
                    </li>
                    <li>
                        <a href="javascript:" class="inline-flex items-center gap-1">
                            <input type="checkbox" id="email" class="form-checkbox" />
                            <label for="email" class="mb-0 cursor-pointer">Email</label>
                        </a>
                    </li>
                </ul>
            </div>
            ...
            <!-- Status Filter -->
            <div>
                <h6 class="text-base font-bold pb-5">Status</h6>
                <ul class="space-y-1.5">
                    <li>
                        <a href="javascript:" class="inline-flex items-center gap-1">
                            <input type="checkbox" id="planned" class="form-checkbox" />
                            <label for="planned" class="mb-0 cursor-pointer">Planned</label>
                        </a>
                    </li>
                    <li>
                        <a href="javascript:" class="inline-flex items-center gap-1">
                            <input type="checkbox" id="in-progress" class="form-checkbox" />
                            <label for="in-progress" class="mb-0 cursor-pointer">In Progress</label>
                        </a>
                    </li>
                    <li>
                        <a href="javascript:" class="inline-flex items-center gap-1">
                            <input type="checkbox" id="completed" class="form-checkbox" />
                            <label for="completed" class="mb-0 cursor-pointer">Completed</label>
                        </a>
                    </li>
                </ul>
            </div>
            ...
            <!-- Priority Filter -->
            <div>
                <h6 class="text-base font-bold pb-5">Priority</h6>
                <ul class="space-y-1.5">
                    <li>
                        <a href="javascript:" class="inline-flex items-center gap-1">
                            <input type="checkbox" id="high" class="form-checkbox" />
                            <label for="high" class="mb-0 cursor-pointer">High</label>
                        </a>
                    </li>
                    <li>
                        <a href="javascript:" class="inline-flex items-center gap-1">
                            <input type="checkbox" id="medium" class="form-checkbox" />
                            <label for="medium" class="mb-0 cursor-pointer">Medium</label>
                        </a>
                    </li>
                    <li>
                        <a href="javascript:" class="inline-flex items-center gap-1">
                            <input type="checkbox" id="low" class="form-checkbox" />
                            <label for="low" class="mb-0 cursor-pointer">Low</label>
                        </a>
                    </li>
                </ul>
            </div>
        ...
        <div class="panel flex-1 overflow-auto">
                ...
                <div class="flex items-center justify-between relative gap-4">
                    <div class="flex items-center">
                        ...
                        <div class="relative border border-white-dark/30 dark:border-[#1b2e4b] rounded-md p-3">
                            <input type="text" placeholder="Search Activities..." class="form-input peer !border-0 !bg-transparent !shadow-none ps-8 !py-0 !h-5" />
                    ...
                    <div class="flex flex-wrap gap-2 items-center">
                        <a href="{{ route('crm.activities.create') }}" class="btn btn-primary">
            ...
            <div class="crm-table pt-14 relative">
                <table id="activitiesTable" class="whitespace-nowrap">
                </table>
            </div>
        </div>
    </div>
    ...
    document.addEventListener("alpine:init", () => {
        Alpine.data("activitiesList", () => ({
            ...
            async loadActivities() {
                try {
                    const response = await fetch('{{ route("crm.api.activities") }}');
            ...
            initializeTable() {
                this.datatable = new simpleDatatables.DataTable('#activitiesTable', {
                    data: {
                        headings: [
                            'ID',
                            "Type",
                    ...
                    columns: [
                        {
                            ...
                            render: function(data, cell, row) {
                                ...
                                return `<div class="flex items-center gap-2">
                                    <div class="p-2 ${bgClass} rounded-full">
                                        <span class="font-bold ${iconClass} text-xl">${data.charAt(0)}</span>
                                    </div>
                                    <div>
                                        <span>${data}</span>
                                    </div>
                        ...
                        },
                        {
                            select: 2,
                            render: function(data, cell, row) {
                                return `<a href="/crm/activities/${row.cells[0].data}" class="font-semibold text-primary hover:underline">${data}</a>`;
                            },
                        },
                        {
                            select: 4,
                            render: function(data, cell, row) {
                                let statusClass = '';
                                if (data === 'Planned') statusClass = 'bg-info/10 text-info dark:bg-info/20';
                                else if (data === 'In Progress') statusClass = 'bg-warning/10 text-warning dark:bg-warning/20';
                                else if (data === 'Completed') statusClass = 'bg-success/10 text-success dark:bg-success/20';
                        ...
                        },
                        {
                            ...
                            render: function(data, cell, row) {
                                let priorityClass = '';
                                if (data === 'High') priorityClass = 'text-danger';
                                else if (data === 'Medium') priorityClass = 'text-warning';
                                else if (data === 'Low') priorityClass = 'text-success';
                                
                                return `<span class="font-semibold ${priorityClass}">${data}</span>`;
                            },
                        },
                        {
                            select: 8,
                            ...
                            render: function(data, cell, row) {
                                return `<ul class="flex gap-2">
                                    <li>
                                        <a href="/crm/activities/${row.cells[0].data}" class="hover:text-info" title="View">
                                    ...
                                    <li>
                                        <a href="/crm/activities/${row.cells[0].data}/edit" class="hover:text-primary" title="Edit">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
                                    ...
                                    <li>
                                        <a href="javascript:;" class="hover:text-success" title="Mark Complete" @click="completeActivity(${row.cells[0].data})">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
                                                <path opacity="0.5" d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z" stroke="currentColor" stroke-width="1.5"></path>
                                                <path d="M8.5 12.5L10.5 14.5L15.5 9.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                            </svg>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="javascript:;" class="hover:text-danger" title="Delete" @click="deleteActivity(${row.cells[0].data})">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
            ...
            setTableData() {
                ...
                for (let i = 0; i < this.activities.length; i++) {
                    dataArr.push([
                        ...
                        this.activities[i].subject,
                        this.activities[i].due_date,
                        this.activities[i].status,
                        this.activities[i].priority,
                        this.activities[i].related_to,
                        this.activities[i].assigned_to,
                        ''
                    ]);
                }
