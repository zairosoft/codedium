...
@section('title', 'แก้ไขอนุโมทนาบัตร')
@section('script')
...
@section('content')
    <div>
        ...
        <div class="mb-5 flex flex-wrap items-center justify-between gap-4">
            <div class="text-lg font-semibold ltr:sm:text-left rtl:sm:text-right dark:text-white-light">แก้ไขอนุโมทนาบัตร</div>
         ...
         <form action="{{ route('intentform.update', $intentform->id) }}" method="POST" x-data="invoiceAdd">
            @csrf
            @method('PUT')
                ...
                <div class="panel flex-1 px-0 py-6 ltr:lg:mr-6 rtl:lg:ml-6">
                    <div class="flex flex-wrap justify-between px-4">
                        ...
                        <div class="w-full lg:w-1/2 lg:max-w-fit">
                            ...
                            <div class="mt-4 flex items-center">
                                ...
                                <input id="date" type="date" name="date" class="form-input w-2/3 lg:w-[250px]" required value="{{ old('date', $intentform->date) }}" />
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="mb-6 w-full lg:w-1/2 ltr:lg:mr-6 rtl:lg:ml-6">
                                ...
                                <div class="mt-4 flex items-center">
                                    ...
                                    <input
                                        ...
                                        value="{{ old('account_name', $intentform->account_name) }}"
                                    />
                                </div>
                                <div class="mt-4 flex items-center">
                                    ...
                                    <input
                                        ...
                                        value="{{ old('account_number', $intentform->account_number) }}"
                                    />
                                </div>
                                <div class="mt-4 flex items-center">
                                ...
                                <div class="mt-4 flex items-center">
                                    ...
                                    <input
                                        ...
                                        value="{{ old('payee', $intentform->payee) }}"
                                    />
                                </div>
                            </div>
                            <div class="w-full lg:w-1/2">
                                ...
                                <div class="mt-4 flex items-center">
                                    ...
                                    <input
                                        ...
                                        value="{{ old('account_bank', $intentform->account_bank) }}"
                                    />
                                </div>
                                <div class="mt-4 flex items-center">
                                    ...
                                    <input
                                        ...
                                        value="{{ old('refer', $intentform->refer) }}"
                                    />
                                </div>
                                <div class="mt-4 flex items-center">
                                    ...
                                    <input
                                        ...
                                        name="foundation"
                                        class="form-input flex-1"
                                        value="{{ old('foundation', $intentform->foundation) }}"
                    ...
                    <div class="mt-8 px-4">
                        <div>
                            ...
                            >{{ old('notes', $intentform->notes) }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="panel mb-5">
                        <div>
                            ...
                            <select id="status" name="status" class="form-select" required>
                                <option value="1" {{ old('status', $intentform->status) == '1' ? 'selected' : '' }}>ใช้งาน</option>
                                <option value="0" {{ old('status', $intentform->status) == '0' ? 'selected' : '' }}>ไม่ใช้งาน</option>
                            </select>
                        </div>
                        <div class="mt-4">
                            <label for="payment-method">ช่องทางการชำระ</label>
                            <select id="payment-method" name="payment_methods" class="form-select" required>
                                <option value="เงินสด" {{ old('payment_methods', $intentform->payment_methods) == 'เงินสด' ? 'selected' : '' }}>เงินสด</option>
                                <option value="เงินโอน" {{ old('payment_methods', $intentform->payment_methods) == 'เงินโอน' ? 'selected' : '' }}>เงินโอน</option>
                            </select>
                        </div>
                    </div>
                    <div class="panel">
        ...
        document.addEventListener('alpine:init', () => {
                Alpine.data('invoiceAdd', () => ({
                    ...
                    init() {
                        ...
                        @foreach($intentform->donations as $donation)
                            this.items.push({
                                id: {{ $donation->id }},
                                type_id: '{{ $donation->type_id }}',
                                title: '{{ $donation->type->name ?? '' }}',
