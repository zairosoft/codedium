...
@section('title', 'รายงานอนุโมทนาบัตร')
@section('content')
    <div x-data="reportFilter">
        <div class="mb-5 flex flex-wrap items-center justify-between gap-4">
            <div class="text-lg font-semibold ltr:sm:text-left rtl:sm:text-right dark:text-white-light">รายงานอนุโมทนาบัตร
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-2xl font-bold">{{ count($intentforms) }}</div>
                    </div>
                </div>
        ...
        <div class="panel">
            <div class="mb-5">
                <h5 class="text-lg font-semibold">รายการอนุโมทนาบัตร</h5>
            </div>
                ...
                <table class="table-hover">
                    ...
                    <tbody>
                        @forelse($intentforms as $index => $intentform)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>{{ sprintf('%03d', $intentform->volume) }}/{{ sprintf('%03d', $intentform->number) }}</td>
                                <td>{{ \Carbon\Carbon::parse($intentform->date)->format('d/m/Y') }}</td>
                                <td>{{ $intentform->name }}</td>
                                <td>
                                    @if($intentform->donations->count() > 0)
                                        <ul class="list-disc list-inside text-sm">
                                            @foreach($intentform->donations as $donation)
                                                <li>{{ $donation->type->name ?? '-' }} ({{ $donation->quantity }})</li>
                                            @endforeach
                                        </ul>
                                    @else
                                        -
                                ...
                                <td>
                                    <div class="flex gap-2">
                                        ...
                                        <a href="{{ route('intentform.print', $intentform->id) }}" target="_blank"
                                            class="text-info" title="พิมพ์">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
                                                <path
                                                ...
                                                <path d="M18 14V19C18 20.1046 17.1046 21 16 21H8C6.89543 21 6 20.1046 6 19V14"
                                                    stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
                                                <path opacity="0.5"
                                                    d="M17.9998 6V5C17.9998 3.58579 17.9998 2.87868 17.56 2.43934C17.1207 2 16.4136 2 14.9993 2H8.99977C7.58556 2 6.87845 2 6.43911 2.43934C5.99977 2.87868 5.99977 3.58579 5.99977 5V6"
                                                    stroke="currentColor" stroke-width="1.5" />
                                            </svg>
                                        </a>
                                    </div>
