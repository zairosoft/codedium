...
@section('title', 'เพิ่มประเภทการบริจาค')
@section('content')
    <div>
        <div class="mb-5 flex flex-wrap items-center justify-between gap-4">
            <div class="text-lg font-semibold ltr:sm:text-left rtl:sm:text-right dark:text-white-light">เพิ่มประเภทการบริจาค
            </div>
        ...
        <div class="panel">
            <form action="{{ route('intentform.types.store') }}" method="POST">
                @csrf
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input id="name" type="text" name="name" class="form-input @error('name') border-danger @enderror"
                            placeholder="กรอกชื่อประเภทการบริจาค" value="{{ old('name') }}" required />
                        @error('name')
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="3"
                            ...
                            placeholder="กรอกคำอธิบาย (ถ้ามี)">{{ old('description') }}</textarea>
                        @error('description')
                            <span class="text-danger text-xs">{{ $message }}</span>
                        @enderror
                    </div>
