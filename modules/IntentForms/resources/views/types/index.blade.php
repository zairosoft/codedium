...
@section('title', 'จัดการประเภทการบริจาค')
@section('style')
    <style>
        .btn-add {
            right: 230px;
...
@section('content')
    ...
    <div x-data="sorting">
        <div class="mb-5 flex flex-wrap items-center justify-between gap-4">
            <div class="text-lg font-semibold ltr:sm:text-left rtl:sm:text-right dark:text-white-light">ประเภทการบริจาค
            </div>
            <div class="flex flex-wrap items-center justify-between gap-4">
                <ul class="flex text-gray-500 dark:text-white-dark">
                    ...
                    <li class="before:content-['/'] before:px-1.5"><a href="javascript:;"
                            class="text-black dark:text-white-light hover:text-black/70 dark:hover:text-white-light/70">ประเภทการบริจาค</a>
                    </li>
                </ul>
            </div>
        </div>
                ...
                <div class="px-5">
                    <div class="md:absolute btn-add">
                        <div class="flex items-center">
                            <a href="{{ route('intentform.types.create') }}" class="btn btn-primary">
            ...
            Alpine.data('sorting', () => ({
                ...
                items: [
                    @foreach ($types as $key => $value)
                        ...
                        no: {{ $key + 1 }},
                        name: '{{ $value->name }}',
                        price: '{{ number_format($value->price, 2) }} บาท',
                ...
                initializeTable() {
                    this.datatable = new simpleDatatables.DataTable('#myTable', {
                        data: {
                            headings: ["ลำดับ", "ชื่อ", "ราคา", "คำอธิบาย", "{{ __('others.view') }}@can('intentform update') / {{ __('others.edit') }}@endcan @can('intentform delete') / {{ __('others.delete') }}@endcan"],
                            data: this.dataArr,
                        },
                        perPage: 20,
                ...
                deleteItem() {
                    ...
                    fetch("{{ route('intentform.types.delete') }}", {
                        method: "DELETE",
                        headers: {
