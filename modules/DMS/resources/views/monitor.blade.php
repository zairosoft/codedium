    ...
    <div>
        <div class="relative flex h-full gap-5 sm:h-[calc(100vh_-_150px)] sm:min-h-0">
            <div class="panel absolute z-10 hidden w-full max-w-xs flex-none space-y-4 overflow-hidden p-4 xl:relative xl:block xl:h-full">
                <div class="relative">
                    <input type="text" class="peer form-input ltr:pr-9 rtl:pl-9" id="filterCards" placeholder="ค้นหา..." />
                ...
                <div class="!mt-0">
                    <div class="chat-users perfect-scrollbar relative -mr-3.5 h-full min-h-[100px] space-y-0.5 pr-3.5 sm:h-[calc(100vh_-_357px)]">
                        <div class="flex flex-col rounded-md">
                            ...
                            <div class="card-dms cursor-pointer flex border-b border-[#e0e6ed] px-4 py-2.5 hover:bg-[#eee] dark:border-[#1b2e4b] dark:hover:bg-[#eee]/10" onclick="openMonitor('{{$value->device_id}}')">
                                ...
                                <div class="flex-1 font-semibold">
                                    @if ($value->alarm != null)
                                    <h6 class="mb-1 mt-1 text-danger searchable">{{ $value->device_name }} ({{ $value->device_id }})</h6>
                                    @else
                                    <h6 class="mb-1 mt-1 searchable" id="alarm_{{ $value->device_id }}">{{ $value->device_name }} ({{ $value->device_id }})</h6>
                                    @endif
...
<script>
    ...
    $("#filterCards").keyup(function () {
        ...
        if ($i > 0) {
            $(this).closest(".card-dms").show();
        } else {
            $(this).closest(".card-dms").hide();
        }
        });
    });
    });
