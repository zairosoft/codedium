...
<div>
    ...
    <div class="panel">
        ...
        <form action="{{ route('dms.update') }}" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex flex-col sm:flex-row">
                <div class="ltr:sm:mr-4 rtl:sm:ml-4 w-full sm:w-2/12 mb-5">
                    <img src="{{ asset($dms->img === null ? 'modules/dms/images/user.png' : 'modules/dms/images/' . $dms->img) }}" alt="image" class="w-20 h-20 md:w-32 md:h-32 rounded-full object-cover mx-auto" id="preview" />
                ...
                <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input id="name" name="name" type="text" value="{{ old('name', $dms->name) }}" placeholder="ป้อนชื่อ" class="form-input @error('name')is-invalid @enderror" />
                        @if ($errors->has('name'))
                            <div>
                    ...
                    <div>
                        ...
                        <div>
                            <input id="device_id" type="text" value="{{ old('tel', $dms->tel) }}" name="tel" placeholder="099 999 9999" x-mask="099 999 9999" class="form-input" />
                        </div>
                    </div>
                    <div>
                        <label for="device_name">ชื่ออุปกรณ์</label>
                        <div>
                            <input id="device_name" type="text" value="{{ old('device_name', $dms->device_name) }}" name="device_name" placeholder="ป้อนชื่ออุปกรณ์" class="form-input" />
                        </div>
                    </div>
                    <div>
                        <label for="device_id">รหัสอุปกรณ์</label>
                        <div>
                            <input id="device_id" type="text" name="device_id" value="{{ old('device_id', $dms->device_id) }}" placeholder="ป้อนรหัสอุปกรณ์" class="form-input" />
                        </div>
                    </div>
                    <div>
                        <label for="car_plate_number">ป้ายทะเทียน</label>
                        <div>
                            <input id="car_plate_number" type="text" name="car_plate_number" value="{{ old('car_plate_number', $dms->car_plate_number) }}" placeholder="ป้อนป้ายทะเทียน" class="form-input" />
                        </div>
                    </div>
                    <div>
                        <label for="car_plate_number_sub">ป้ายทะเทียนหาง</label>
                        <div>
                            <input id="car_plate_number_sub" type="text" name="car_plate_number_sub"  value="{{ old('car_plate_number_sub', $dms->car_plate_number_sub) }}" placeholder="ป้อนป้ายทะเทียน" class="form-input" />
                        </div>
                    </div>
                ...
                <div class="flex-1 grid grid-cols-1 sm:grid-cols-1 gap-5">
                    <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <div>
                                <input id="sim_number" type="text" name="sim_number" value="{{ old('sim_number', $dms->sim_number) }}" placeholder="099 999 9999" x-mask="099 999 9999" class="form-input" />
                            </div>
                        </div>
                        <div>
                            ...
                            <div>
                                <select class="form-select text-white-dark selectize" name="sim_network" id="sim_network">
                                    ...
                                    <option value="ais"{{ $dms->sim_network == 'ais' ? 'selected' : '' }}>AIS</option>
                                    <option value="true"{{ $dms->sim_network == 'true' ? 'selected' : '' }}>True</option>
                                    <option value="dtac"{{ $dms->sim_network == 'dtac' ? 'selected' : '' }}>Dtac</option>
                                    <option value="nt"{{ $dms->sim_network == 'nt' ? 'selected' : '' }}>NT</option>
                                </select>
                            </div>
                        </div>
                        <div>
                            ...
                            <div>
                                <input id="start_date" type="date" name="start_date" value="{{ old('start_date', $dms->start_date) }}" placeholder="ป้อนวันเริ่มต้น" class="form-input" />
                            </div>
                        </div>
                        <div>
                            <label for="end_date">วันสิ้นสุด</label>
                            <div>
                                <input id="end_date" type="date" name="end_date" value="{{ old('end_date', $dms->end_date) }}" placeholder="ป้อนวันสิ้นสุด" class="form-input" />
                            </div>
                        </div>
                        <div>
                            ...
                            <div>
                                <select class="form-select text-white-dark selectize" name="sim_type" id="sim_type">
                                    ...
                                    <option value="1"{{ $dms->sim_type == 1 ? 'selected' : '' }}>รายปี</option>
                                    <option value="2"{{ $dms->sim_type == 2 ? 'selected' : '' }}>รายเดือน</option>
                                    <option value="3"{{ $dms->sim_type == 3 ? 'selected' : '' }}>รายสัปดาห์</option>
                                    <option value="4"{{ $dms->sim_type == 4 ? 'selected' : '' }}>รายวัน</option>
                                    <option value="5"{{ $dms->sim_type == 5 ? 'selected' : '' }}>ตลอดอายุการใช้งาน</option>
                                </select>
                            </div>
                        </div>
                        <div>
                            ...
                            <div>
                                <input id="other" type="text" name="other" value="{{ old('other', $dms->other) }}" placeholder="ป้อนข้อมูล" class="form-input" />
                            </div>
                        </div>
                    </div>
