<div class="mb-6 grid gap-6 lg:grid-cols-3">
    <div class="h-full p-0">
        ...
        @endforeach
        <div class="rounded-md border border-gray-500/20 p-2 mt-2">
            @foreach ($logs as $key => $value)
            <div class="mb-2 flex items-center font-semibold">
                @if ($value->type == 'actionEye')
                    <div class="grid h-8 w-8">
                        <img src="{{ Module::asset('dms:images/eyelash.png') }}" alt="actionEye">
                    </div>
                    <div class="ltr:ml-2 rtl:mr-2">
                        <h6 class="text-dark dark:text-white-light">ผู้ขับขี่หลับตา</h6>
                        <p class="text-xs text-white-dark">actionEye {{ date("d/m/Y H:i", strtotime($value->created_at)) }}</p>
                    </div>
                    @elseif ($value->type == 'actionLooking')
                    <div class="grid h-8 w-8">
                        <img src="{{ Module::asset('dms:images/road.png') }}" alt="actionLooking">
                    </div>
                    <div class="ltr:ml-2 rtl:mr-2">
                        <h6 class="text-dark dark:text-white-light">ผู้ขับขี่ละสายตาจากด้านหน้า</h6>
                        <p class="text-xs text-white-dark">actionLooking {{ date("d/m/Y H:i", strtotime($value->created_at)) }}</p>
                    </div>
                    @elseif ($value->type == 'actionYawning')
                    <div class="grid h-8 w-8">
                        <img src="{{ Module::asset('dms:images/yawn.png') }}" alt="actionYawning">
                    </div>
                    <div class="ltr:ml-2 rtl:mr-2">
                        <h6 class="text-dark dark:text-white-light">ผู้ขับขี่หาว</h6>
                        <p class="text-xs text-white-dark">actionYawning {{ date("d/m/Y H:i", strtotime($value->created_at)) }}</p>
                    </div>
                    @elseif ($value->type == 'actionPhone')
                    <div class="grid h-8 w-8">
                        <img src="{{ Module::asset('dms:images/phone.png') }}" alt="actionPhone">
                    </div>
                    <div class="ltr:ml-2 rtl:mr-2">
                        <h6 class="text-dark dark:text-white-light">ผู้ขับขี่ใช้โทรศัพท์มือถือ</h6>
                        <p class="text-xs text-white-dark">actionPhone {{ date("d/m/Y H:i", strtotime($value->created_at)) }}</p>
                    </div>
                    @elseif ($value->type == 'actionCigarette')
                    <div class="grid h-8 w-8">
                        <img src="{{ Module::asset('dms:images/cigarette.png') }}" alt="actionCigarette">
                    </div>
                    <div class="ltr:ml-2 rtl:mr-2">
                        <h6 class="text-dark dark:text-white-light">ผู้ขับขี่สูบบุหรี่</h6>
                        <p class="text-xs text-white-dark">actionCigarette {{ date("d/m/Y H:i", strtotime($value->created_at)) }}</p>
                    </div>
                @endif
            </div>
            @endforeach
