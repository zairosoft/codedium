        ...
        Alpine.data('sorting', () => ({
            ...
            init() {
                this.datatable = new simpleDatatables.DataTable('#myTable', {
                    data: {
                        headings: ['ลำดับ', 'รหัสอุปกรณ์', 'ประเภท', 'รายละเอียด', 'ละติจูด', 'ลองจิจูด', 'ความเร็ว'],
                        data: [
                            @foreach ($logs as $key => $value)
                            [{{ (int)$key + 1 }}, '{{ $value->device_id }}', '{{ $value->type }}', '{{ $value->detail }}', '{{ $value->latitude }}', '{{ $value->longitude }}', '{{ $value->speed }} km/h'],
                            @endforeach
                        ],
                    },
                    searchable: true,
