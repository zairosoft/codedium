            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
                <ul class="flex text-gray-500 dark:text-white-dark">
                    ...
                    <li class="before:content-['/'] before:px-1.5"><a href="javascript:;"
                            class="text-black dark:text-white-light hover:text-black/70 dark:hover:text-white-light/70">ค่าใช้จ่าย</a>
                    </li>
                </ul>
            </div>
        </div>
            ...
            Alpine.data('sorting', () => ({
                ...
                items: [
                    @foreach ($expenses as $key => $value)
                                        {
                        no: {{ $key + 1 }},
                        date: '{{ $value->date ? $value->date->format("d/m/Y") : "-" }}',
                        due_date: '{{ $value->due_date ? $value->due_date->format("d/m/Y") : "-" }}',
                ...
                initializeTable() {
                    this.datatable = new simpleDatatables.DataTable('#myTable', {
                        data: {
                            headings: ["ลำดับ", "วันที่", "วันครบกำหนด", "เลขที่อ้างอิง", "ผู้รับเงิน", "ยอดสุทธิ", "สถานะ", "{{ __('others.view') }}@can('expenses update') / {{ __('others.edit') }}@endcan @can('expenses delete') / {{ __('others.delete') }}@endcan"],
                            data: this.dataArr,
                        },
                        perPage: 20,
                ...
                searchItems() {
                    return this.items.filter((d) => (d.payee && d.payee.toLowerCase().includes(this.searchText)));
                },
                deleteConfirm(item) {
                    setTimeout(() => {
                        this.itemID = item;
